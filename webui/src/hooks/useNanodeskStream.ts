...
export function useNanodeskStream(
  chatId: string | null,
  ...
  useEffect(() => {
    ...
    const handle = (ev: InboundEvent) => {
      if (ev.event === "error") {
        ...
        } else if (ev.detail === "agent_rejected") {
          applyStreamError({
            kind: "agent_rejected",
            reason: ev.reason,
            chatId,
          ...
          return [
            ...prev,
            {
              ...
              ...(ev.session_mentions?.length
                ...
                : {}),
              ...(ev.path_refs?.length ? { pathRefs: ev.path_refs } : {}),
            },
          ];
        });
        ...
        return [
          ...(sideChannel || continueActiveTurn ? base : pruneReasoningOnlyPlaceholders(base)),
          {
            ...
            ...(options?.sessionMentions?.length
              ...
              : {}),
            ...(options?.pathRefs?.length ? { pathRefs: options.pathRefs } : {}),
          },
        ];
      });
