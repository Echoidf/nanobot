...
import type { ChannelRuntimeStatus, NanodeskFeatureInfo } from "@/lib/types";
...
export function channelSetup(
  ...
  const presentation: ChannelSetupPresentation = {
    ...
    summary:
      ...
      ?? "Enable turns on this channel in nanodesk, but this integration still needs platform-specific setup before it can receive messages.",
    tryIt: setupMessages?.tryIt,
    steps: setupMessages?.steps ?? [
      `Open ~/.nanodesk/config.json and find channels.${feature.name}.`,
      "Add the credentials required by that platform, using the channel documentation as the source of truth.",
      "Restart nanodesk, then send a small test message from that platform.",
    ],
...
export function channelDisplayName(feature: NanodeskFeatureInfo): string {
...
export function channelDescription(feature: NanodeskFeatureInfo, t: ReturnType<typeof useTranslation>["t"]): string {
  const fallback =
    `Use nanodesk from ${channelDisplayName(feature)}.`;
  return channelTranslator(t, channelUiOwner(feature.name))("description", fallback);
}

export function channelRequirements(feature: NanodeskFeatureInfo, t: ReturnType<typeof useTranslation>["t"]): string {
  const fallback =
    "Channel credentials and gateway settings";
  return channelTranslator(t, channelUiOwner(feature.name))("requirements", fallback);
}

export function channelMatchesFilter(feature: NanodeskFeatureInfo, filter: ChannelFilter): boolean {
  if (filter === "on") return channelIsRunning(feature);
  if (filter === "off") return !channelIsRunning(feature);
  return true;
}

export function channelIsRunning(feature: NanodeskFeatureInfo): boolean {
  return feature.runtime_status === "running";
}

export function channelToggleChecked(feature: NanodeskFeatureInfo): boolean {
