...
const WS_OPEN = 1;
const WS_CLOSING = 2;
const HOST_SOCKET_URL_PREFIX = "nanodesk-host://";
 ...
 * - **Production**: off unless ``localStorage.setItem('nanodesk_debug_ws','1')`` (or ``true``).
 * - **Silence anywhere**: ``localStorage.setItem('nanodesk_debug_ws','0')`` (or ``false`` / ``off``).
...
function wsInboundDebugEnabled(): boolean {
  ...
  try {
    ...
    const raw = ls?.getItem("nanodesk_debug_ws")?.trim().toLowerCase() ?? "";
...
export interface NanodeskClientOptions {
  url: string;
  reconnect?: boolean;
...
export class NanodeskClient {
  ...
  constructor(private options: NanodeskClientOptions) {
  ...
  reconcileCanonicalCompletion(
    ...
    while (fences.size > NanodeskClient.COMPLETED_TURN_FENCE_MAX) {
  ...
  sendMessage(
    ...
    const frame: Outbound = {
      ...
      ...(options?.sessionMentions?.length
        ...
        : {}),
      ...(options?.pathRefs?.length ? { path_refs: options.pathRefs } : {}),
  ...
  private handleMessage(ev: MessageEvent): void {
    ...
    try {
      ...
      if (wsInboundDebugEnabled()) {
        ...
        console.warn(
          "[nanodesk ws inbound] invalid JSON",
    ...
    if (wsInboundDebugEnabled()) {
      console.log("[nanodesk ws inbound]", summarizeInboundWsPayload(parsed));
    }
    ...
    if (parsed.event === "session_updated") {
      this.emitSessionUpdate(parsed.chat_id, parsed.scope, parsed.workspace_scope, parsed.agent_id);
      return;
    }
  ...
  private emitSessionUpdate(
    ...
    for (const handler of this.sessionUpdateHandlers) {
      handler(chatId, scope, workspaceScope, agentId);
    }
  }
  ...
  private dispatch(chatId: string, ev: InboundEvent): void {
    ...
    q.push(ev);
    const over = q.length - NanodeskClient.PENDING_INBOUND_MAX;
    if (over > 0) {
      q.splice(0, over);
    }
  }
