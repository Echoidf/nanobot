...
export interface NanodeskHostApi {
...
const LOOPBACK_HOST_STORAGE_KEY = "nanodesk-webui.native-host";
...
let loopbackHostApi: NanodeskHostApi | null = null;

declare global {
  interface Window {
    nanodeskHost?: NanodeskHostApi;
  }
}

function getHostApi(): NanodeskHostApi | null {
  if (typeof window === "undefined") return null;
  return window.nanodeskHost ?? loopbackHostApi;
}
...
function createLoopbackHostApi(config: LoopbackHostConfig): NanodeskHostApi {
  return {
    async pickFolder(): Promise<string | null> {
      ...
      try {
        ...
        throw new Error("Native folder picker is unavailable. Reopen Nanodesk and try again.");
      }
